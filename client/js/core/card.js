game.card = {
  ...
  build: function(data) {
    ...
    var att = data.attribute;
    if (!att && data.deck == game.data.ui.summon) {
      att = game.data.ui.summon;
    } else if (!att) {
      att = data.type;
    }
    legend = $('<legend>').text(att);
    fieldset = $('<fieldset>');
  ...
  select: function(event) {
    ...
    if (card) {
      if (forceSelection && card.length) game.card.setSelection(card);
      else if (!game.lockSelection && !card.hasClasses('selected attacktarget casttarget dead')) {
        if (event && event.button !== 2 && card.length) game.card.setSelection(card, event);
